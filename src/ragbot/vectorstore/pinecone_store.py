...
class PineconeStore:
    ...
    def add_documents(self, documents, ids=None):  # ← ADD ids PARAMETER
        ...
        If ids are provided, uses them for idempotent upserts (no duplicates).
        """
        logger.info(f"Embedding {len(documents)} documents")
        
        # Pass ids to the underlying vectorstore
        self.vectorstore.add_documents(documents, ids=ids, batch_size=100)
